...
app.post("/todos", async (req, res) => {
    try {
        ...
        const {name} = req.body;
        const newTodo = await pool.query("INSERT INTO users (name) VALUES($1) RETURNING *;", [name]);
...
app.get("/todos/:id", async (req, res) => {
    try {
        ...
        const {id} = req.params;
        const oneTodo = await pool.query("SELECT * FROM users WHERE id=$1;", [id]);
...
app.put("/todos/:id", async (req, res) => {
    try {
        ...
        const deleteOneTodo = await pool.query("UPDATE users SET name=$1 WHERE id=$2;", [name, id]);
...
app.delete("/todos/:id", async (req, res) => {
    try {
        console.log(req.params); //id
        const {id} = req.params;
        const deleteOneTodo = await pool.query("DELETE FROM users WHERE id=$1;", [id]);
